...
void version(void)
{
   fputs(STRING_DOS_COREUTILS_VERSION, stdout);
}
#endif
...
#define emit_try_help()  \
   do  \
   {  \
      fprintf(stderr, STRING_EMIT_TRY_HELP, PROGRAM_NAME);  \
   }  \
   while(0)
...
{
   fputs("", stdout);
   fputs(STRING_EMIT_STDIN_NOTE, stdout);
}

static inline void emit_mandatory_arg_note(void)
{
   fputs("", stdout);
   fputs(STRING_EMIT_MANDATORY_ARG_NOTE, stdout);
}
