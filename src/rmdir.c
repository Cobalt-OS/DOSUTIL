...
#include <sys/types.h>
#include <io.h> /* _findfile functions */
#include <stdbool.h>
#include "system.h"
#include "..\lib\error.h"
#include "..\lib\errno.h"
#include "..\lib\getopt.h"
#include "..\lib\dosname.h" /*IS_RELATIVE_FILE_NAME macro */
#include "pfprintf.h"
...
/* This  function determines if a directory is empty
is_empty_dir is normally found in system.h
as of this writing 7-7-2018 openat is not implemented.
Portitions of this function may eventually be moved to sytem.h
*/
bool
is_empty_dir(const char *dir){
...
if( IS_RELATIVE_FILE_NAME(dir) ){ // processing AT_FDCWD  

   getcwd(cwd, PATH_MAX);
   if(errno == 0){
        strncpy(final_path, cwd, PATH_MAX);
        strncat(final_path, "\\", 1);
        strncat(final_path, dir, PATH_MAX);
   }
   else { // getcwd failed
        strncat(final_path, dir, PATH_MAX);
        strncat(final_path, "\\", 1);
...
else { // Working with an absolute path
   strncat(final_path, dir, PATH_MAX);
   strncat(final_path, "\\", 1);
   }

handle = _findfirst(final_path, &fileinfo);
rc = handle;

while( rc != -1) /* loop through the files in final_path */
{
        rc = _findnext(handle, &fileinfo );
        if(  !((strcmp(fileinfo.name, ".") == 0) || strcmp(fileinfo.name,"..") == 0)){    /* ignore . and .. */              
        dir_is_empty = false;
        break;
        }
}
  ...
  return (ignore_fail_on_non_empty
          && (errno_rmdir_non_empty (error_number)
              || (errno_may_be_empty (error_number)
                  && is_empty_dir (dir)))); //old code && is_empty_dir (AT_FDCWD, dir))));
}
  ...
  while (1)
    {
      ...
      if (verbose)
        prog_fprintf (stdout, _("removing directory, %s"), dir);//quoteaf (dir));
      ...
      if (!ok)
        {
          ...
          if (ignorable_failure (errno, dir))
          ...
          else
            {
              ...
              error (0, errno, _("failed to remove directory %s"),dir);//quoteaf (dir));
            }
          break;
        }
    }
      ...
      if (rmdir (dir) != 0)
        {
          ...
          error (0, errno, _("failed to remove %s"), dir);//quoteaf (dir));
          ok = false;
        }
